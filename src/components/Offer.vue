<template>
    <section class="offer">
        <div class="offer__content container">
            <div class="offer__content-wrap">
                <div class="offer__wrap-flex">
                    <div class="offer__flex-bg">
                        <svg width="41" height="40" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_859_3966)">
                                <path d="M12.1668 31.6667C14.0078 31.6667 15.5002 30.1743 15.5002 28.3333C15.5002 26.4924 14.0078 25 12.1668 25C10.3259 25 8.8335 26.4924 8.8335 28.3333C8.8335 30.1743 10.3259 31.6667 12.1668 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M28.8333 31.6667C30.6743 31.6667 32.1667 30.1743 32.1667 28.3333C32.1667 26.4924 30.6743 25 28.8333 25C26.9924 25 25.5 26.4924 25.5 28.3333C25.5 30.1743 26.9924 31.6667 28.8333 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.8335 28.3335H7.50016C6.39559 28.3335 5.50016 27.4381 5.50016 26.3335V21.6668M3.8335 8.3335H20.1668C21.2714 8.3335 22.1668 9.22893 22.1668 10.3335V28.3335M15.5002 28.3335H25.5002M32.1668 28.3335H33.5002C34.6047 28.3335 35.5002 27.4381 35.5002 26.3335V18.3335M35.5002 18.3335H22.1668M35.5002 18.3335L31.0828 10.9712C30.7213 10.3688 30.0703 10.0002 29.3678 10.0002H22.1668" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.5 28H7.16667C6.0621 28 5.16667 27.1046 5.16667 26V21.3333M3.5 8H19.8333C20.9379 8 21.8333 8.89543 21.8333 10V28M15.5 28H25.1667M32.5 28H33.1667C34.2712 28 35.1667 27.1046 35.1667 26V18M35.1667 18H21.8333M35.1667 18L30.7493 10.6377C30.3878 10.0353 29.7368 9.66667 29.0343 9.66667H21.8333" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 11.8184H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M2.31836 15.4546H8.98503" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 19.0908H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </g>
                            <defs>
                                <clipPath id="clip0_859_3966">
                                <rect width="40" height="40" fill="white" transform="translate(0.5)" />
                                </clipPath>
                            </defs>
                        </svg>
                    </div>
                    <div class="offer__flex-info">
                        <h5 class="offer__info-title">FREE AND FAST DELIVERY</h5>
                        <p class="offer__info-text">Free delivery for all orders over $140</p>
                    </div>
                </div>
                <div class="offer__wrap-flex">
                    <div class="offer__flex-bg">
                        <svg width="41" height="40" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_859_3966)">
                                <path d="M12.1668 31.6667C14.0078 31.6667 15.5002 30.1743 15.5002 28.3333C15.5002 26.4924 14.0078 25 12.1668 25C10.3259 25 8.8335 26.4924 8.8335 28.3333C8.8335 30.1743 10.3259 31.6667 12.1668 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M28.8333 31.6667C30.6743 31.6667 32.1667 30.1743 32.1667 28.3333C32.1667 26.4924 30.6743 25 28.8333 25C26.9924 25 25.5 26.4924 25.5 28.3333C25.5 30.1743 26.9924 31.6667 28.8333 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.8335 28.3335H7.50016C6.39559 28.3335 5.50016 27.4381 5.50016 26.3335V21.6668M3.8335 8.3335H20.1668C21.2714 8.3335 22.1668 9.22893 22.1668 10.3335V28.3335M15.5002 28.3335H25.5002M32.1668 28.3335H33.5002C34.6047 28.3335 35.5002 27.4381 35.5002 26.3335V18.3335M35.5002 18.3335H22.1668M35.5002 18.3335L31.0828 10.9712C30.7213 10.3688 30.0703 10.0002 29.3678 10.0002H22.1668" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.5 28H7.16667C6.0621 28 5.16667 27.1046 5.16667 26V21.3333M3.5 8H19.8333C20.9379 8 21.8333 8.89543 21.8333 10V28M15.5 28H25.1667M32.5 28H33.1667C34.2712 28 35.1667 27.1046 35.1667 26V18M35.1667 18H21.8333M35.1667 18L30.7493 10.6377C30.3878 10.0353 29.7368 9.66667 29.0343 9.66667H21.8333" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 11.8184H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M2.31836 15.4546H8.98503" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 19.0908H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </g>
                            <defs>
                                <clipPath id="clip0_859_3966">
                                <rect width="40" height="40" fill="white" transform="translate(0.5)" />
                                </clipPath>
                            </defs>
                        </svg>
                    </div>
                    <div class="offer__flex-info">
                        <h5 class="offer__info-title">FREE AND FAST DELIVERY</h5>
                        <p class="offer__info-text">Free delivery for all orders over $140</p>
                    </div>
                </div>
                <div class="offer__wrap-flex">
                    <div class="offer__flex-bg">
                        <svg width="41" height="40" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_859_3966)">
                                <path d="M12.1668 31.6667C14.0078 31.6667 15.5002 30.1743 15.5002 28.3333C15.5002 26.4924 14.0078 25 12.1668 25C10.3259 25 8.8335 26.4924 8.8335 28.3333C8.8335 30.1743 10.3259 31.6667 12.1668 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M28.8333 31.6667C30.6743 31.6667 32.1667 30.1743 32.1667 28.3333C32.1667 26.4924 30.6743 25 28.8333 25C26.9924 25 25.5 26.4924 25.5 28.3333C25.5 30.1743 26.9924 31.6667 28.8333 31.6667Z" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.8335 28.3335H7.50016C6.39559 28.3335 5.50016 27.4381 5.50016 26.3335V21.6668M3.8335 8.3335H20.1668C21.2714 8.3335 22.1668 9.22893 22.1668 10.3335V28.3335M15.5002 28.3335H25.5002M32.1668 28.3335H33.5002C34.6047 28.3335 35.5002 27.4381 35.5002 26.3335V18.3335M35.5002 18.3335H22.1668M35.5002 18.3335L31.0828 10.9712C30.7213 10.3688 30.0703 10.0002 29.3678 10.0002H22.1668" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M8.5 28H7.16667C6.0621 28 5.16667 27.1046 5.16667 26V21.3333M3.5 8H19.8333C20.9379 8 21.8333 8.89543 21.8333 10V28M15.5 28H25.1667M32.5 28H33.1667C34.2712 28 35.1667 27.1046 35.1667 26V18M35.1667 18H21.8333M35.1667 18L30.7493 10.6377C30.3878 10.0353 29.7368 9.66667 29.0343 9.66667H21.8333" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 11.8184H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M2.31836 15.4546H8.98503" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M5.5 19.0908H12.1667" stroke="#FAFAFA" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </g>
                            <defs>
                                <clipPath id="clip0_859_3966">
                                <rect width="40" height="40" fill="white" transform="translate(0.5)" />
                                </clipPath>
                            </defs>
                        </svg>
                    </div>
                    <div class="offer__flex-info">
                        <h5 class="offer__info-title">FREE AND FAST DELIVERY</h5>
                        <p class="offer__info-text">Free delivery for all orders over $140</p>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>
<style setup>
.offer {
    margin: 140px 0;
}
.offer__content-wrap {
    display: flex;
    align-items: center;
    justify-content: space-between;
    flex-wrap: wrap;
}
.offer__wrap-flex {
    flex: 0 1 33%;
    height: 161px;
    display: flex;
    align-items: center;
    justify-content: space-between;
    flex-direction: column;
    gap: 20px;
}
.offer__flex-bg {
    width: 59px;
    height: 59px;
    margin: 0 auto;
    display: flex;
    align-items: center;
    justify-content: center;
    border-radius: 100%;
    background: #000;
    position: relative;
    z-index: 1;
}
.offer__flex-bg::after {
    content: '';
    width: 80px;
    height: 80px;
    border-radius: 100%;
    background: #2f2e3057;
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    z-index: -1;
}
.offer__info-title {
    text-align: center;
    font-family: "Poppins", sans-serif;
    font-weight: 600;
    font-size: 20px;
    line-height: 1.4;
    color: #000;
}
.offer__info-text {
    margin-top: 10px;
    font-weight: 400;
    font-size: 14px;
    line-height: 1.5;
    text-align: center;
    color: #000;
}
@media (max-width: 425px) {
    .offer__content-wrap {
        flex-direction: column;
        gap: 25px;
    }
} 
</style>